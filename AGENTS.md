...
- Review `.planning/` if present for GSD project state, roadmap, and execution history.
...
- Treat `.ai/` as the context layer and `.planning/` as the execution-state layer.
...
- Keep `.ai/*` and `.planning/*` separated: reusable knowledge belongs in `.ai/`, active project planning belongs in `.planning/`.
